    ...
    def save_data(self):
        ...
        try:
            ...
            with open(filename, 'w', newline='') as file:
                ...
                writer.writerow(["Operator Initials", op_box.get()])

                # Write the name as a row
                writer.writerow(["Sample Name", desc_box.get()])

                # Write the name as a row
                writer.writerow(["Material Code", mat_box.get()])

                # Write the name as a row
                writer.writerow(["Lot #", lot_box.get()])
...
class FreeloaderGUI:
    def __init__(self, freeloader):
        ...
        # Create labels
        label_font = ("Arial", 12)  # Set the font size for labels
    ...
    def update_plot(self):
        ...
        if measurements:
            ...
            self.plot.cla()
            self.plot.plot(timestamps, positions, label='Position')
            self.plot.plot(timestamps, weights, label='Weight')
            self.plot.legend()
            self.plot.set_xlabel('Timestamp')
            self.plot.set_ylabel('Value')
            self.plot.set_title('Freeloader Measurements')
            self.figure.autofmt_xdate()
            self.canvas.draw()

    def start(self):
        """ Method to start the GUI """
        self.window.title("Freeloader GUI")
